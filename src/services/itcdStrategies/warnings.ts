...
const MARCADORES: ReadonlyArray<{ categoria: ItcdWarningCategory; marcador: string }> = [
...
export const ITCD_WARNING_LABELS: Record<ItcdWarningCategory, string> = {
...
const descreveIndice = (valor: number, vigenciaInicio?: string): string =>
    `R$ ${valor.toFixed(2)}${vigenciaInicio ? `, vigência ${vigenciaInicio}` : ''}`;

export const avisoTabelaNaoHomologada = (uf: string): string =>
    `Valor referencial: a tabela de ITCD de ${uf} ${M_TABELA}. Confirme a alíquota vigente na SEFAZ/${uf} antes de usar.`;

export const avisoAliquotaDoacaoNaoHomologada = (uf: string): string =>
    `${M_ALIQUOTA_DOACAO} para ${uf}: valor calculado com a regra de causa mortis.`;

export const avisoIndiceNaoConferido = (
    uf: string,
    nome: string,
    valor: number,
    vigenciaInicio?: string
): string => `Valor de ${nome} (${descreveIndice(valor, vigenciaInicio)}) ${M_INDICE_NAO_CONFERIDO}/${uf}.`;

export const avisoIndiceForaDaVigencia = (
    nome: string,
    valor: number,
    vigenciaInicio?: string
): string =>
    `Valor de ${nome} usado (${descreveIndice(valor, vigenciaInicio)}) ${M_INDICE_FORA_DA_VIGENCIA}. Confirme o índice da competência.`;

export const avisoPrazoDescontoExpirado = (dias: number, dataLimite: string): string =>
    `Prazo de ${dias} dias ${M_PRAZO_EXPIRADO} ${dataLimite}.`;

export const avisoDescontoSemDataDoObito = (): string =>
    `Data do óbito não informada. ${M_PRAZO_SEM_DATA}.`;
 ...
 */
export const classifyItcdWarning = (message?: string): ItcdWarningCategory[] => {
    if (!message) return [];
...
const ORDEM_CATEGORIAS: ReadonlyArray<ItcdWarningCategory> = [
 ...
 */
export const groupItcdWarningCategories = (
    itens: ReadonlyArray<ItcdWarningSource>
): Array<{ categoria: ItcdWarningCategory; ufs: string[] }> => {
    const porCategoria = new Map<ItcdWarningCategory, Set<string>>();

    itens.forEach(item => {
        const categorias = new Set(classifyItcdWarning(item.warningMessage));
        if (item.confiabilidade !== 'HOMOLOGADA') categorias.add('TABELA_NAO_HOMOLOGADA');

        categorias.forEach(categoria => {
            const ufs = porCategoria.get(categoria) || new Set<string>();
