...
const pesquisa = (vigenciaInicio, value, source) => ({
    vigenciaInicio,
    value,
    source,
    conferida: false,
...
const ufrPb = (vigenciaInicio, value) => pesquisa(vigenciaInicio, value, FONTE_UFR_PB);
...
const SERIES = {
    ...
    AP: {
        ...
        vigencias: [],
        bloqueio: 'Índice não obtido em fonte oficial: em 09/09/2026 o portal da SEFAZ-AP (www.sefaz.ap.gov.br) ' +
...
const competencia = (data) => `${data.getFullYear()}-${String(data.getMonth() + 1).padStart(2, '0')}`;
const rotuloMes = (competenciaAlvo) => {
...
exports.fiscalUnitsApi = {
    ...
    getUnit: (uf, referenceDate) => {
        ...
        if (!series || series.vigencias.length === 0)
            return null;
     ...
     */
    listSeriesBloqueadas: () => Object.entries(SERIES)
     ...
     */
    listLacunasMensais: (uf, year) => {
        const series = SERIES[uf];
        if (!series || series.periodicidade !== 'MENSAL' || series.vigencias.length === 0)
            return [];
        ...
        for (let mes = 1; mes <= 12; mes++) {
            ...
            if (!publicados.has(competenciaAlvo))
                lacunas.push(competenciaAlvo);
        }
        return lacunas;
    },
};
